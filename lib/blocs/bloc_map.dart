...
class BlocMap extends Disposable {
	...
	void onFountainsBtnClicked() {
		_updateMapData(
        showFountains: !_mapData.value.showFountains
		);
	}
  ...
  void onBackBtnClick() {
	...
	void _updateMapData({
		...
		_mapData.value = mapData != null
				? mapData.copyWith(
			markers: markers ?? mapData.fountains,
			polygons: polygons ?? mapData.polygons,
			showFountains: showFountains ?? mapData.showFountains,
		)
				: MapData(
				fountains: markers, polygons: polygons, showFountains: showFountains);
	}
	...
	Set<Polygon> _createPolygons(List<Field> fields, {
		isSelected = false,
	}) {
