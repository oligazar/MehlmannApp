...
class BlocMap extends ExceptionHandleable implements Disposable {
  ...
  final _locationUpdates = rx.BehaviorSubject<LatLng>();
  ...
  BlocMap() {
    _prepareData();
    _locationUpdates.debounceTime(Duration(milliseconds: 400)).listen((latLng) {
      _points.add(latLng);
  ...
  void onSubmitComment(int fieldId, String text) async {
    final comment = await _api.createComment(fieldId, text).catchError((e) {
      _exception.add(e);
    });
    if (comment != null) {
  ...
  Future onSendSentence(String sentenceName) async {
    ...
    await _api.createGroup(sentenceName, fieldIds).catchError((e) {
      _exception.add(e);
    });
    _fieldsGroup.clear();
  ...
  Future _onFieldClick(Field field) async {
    if (currentMode != BtnsMode.measureArea &&
      ...
      if (currentMode != BtnsMode.createSentence) {
        ...
        final comments = await _api.fetchComments(field.id).catchError((e) {
          _exception.add(e);
        });
